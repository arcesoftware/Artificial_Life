...
const (
	...
	repulsionDistCheck float32 = 300.0
...
func main() {
	...
	window, err := glfw.CreateWindow(winWidth, winHeight, "Tri-Sphere TDA Dynamics - Refined Glow", nil, nil)
	if err != nil {
		panic(err)
	}
...
func render() {
	...
	for i := 0; i < nParticles; i++ {
		...
		if simData.Beta1[sphereID] > 0 {
			// ** IMPROVEMENT: Lower multiplier from 1.6 to 1.25 for less saturation **
			r := float32(math.Min(float64(col.X()*1.25), 1.0))
			g := float32(math.Min(float64(col.Y()*1.25), 1.0))
			b := float32(math.Min(float64(col.Z()*1.25), 1.0))
			buf[idx+3] = r
			buf[idx+4] = g
			buf[idx+5] = b
		} else {
	...
	// ** IMPROVEMENT: Increased scale for a softer, wider halo **
	gl.BlendFunc(gl.SRC_ALPHA, gl.ONE)
	gl.Uniform1f(locPointScale, 30000.0)
	gl.DrawArrays(gl.POINTS, 0, int32(nParticles))

	// Pass 2: core (standard alpha blending)
	// ** IMPROVEMENT: Decreased scale for a sharper, less washed-out core **
	gl.BlendFunc(gl.SRC_ALPHA, gl.ONE_MINUS_SRC_ALPHA)
	gl.Uniform1f(locPointScale, 12000.0)
	gl.DrawArrays(gl.POINTS, 0, int32(nParticles))
...
void main() {
	...
	// 1. Toned-down Brightness Calculation
	// Base brightness is slightly higher, but the velocity boost is lower
	float bright_boost = smoothstep(0.0, 10.0, vIntensity);
	float bright = 0.65 + bright_boost * 0.6; 

	// 2. Keep Color Hue (less white center)
	// Mix the vColor with white based on velocity boost, instead of simple multiplication,
	// to preserve the initial color better and prevent oversaturation near the core.
	vec3 color_core = mix(vColor * bright, vec3(1.0), bright_boost * 0.1); 

	float pulse = 0.9 + 0.12 * sin(uTime*3.0 + vDistance*0.01);

	vec3 base = color_core * pulse;
