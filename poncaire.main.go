...
func updateParticlesPerformant(dt float64) {
	...
	// Step 1: TDA/Topological Analysis ($\beta_0$ check)
	actualCOMs := [numSpheres]mgl32.Vec3{}
	// This function now updates simData.Beta0 and simData.Connected
	...
	// Track particles activating the Betti-1 force for proxy calculation
	...
	for w := 0; w < numWorkers; w++ {
		...
		go func(start, end int) {
			...
			// Safely accumulate local counts into the shared global array
			for id := 0; id < numSpheres; id++ {
				betaOneActiveCounts[id] += localBetaOneActiveCounts[id]
			}
		}(start, end)
	}
	...
	// Step 4: Calculate Global Betti-1 Proxy and Log Poincaré Polynomial
	for id := 0; id < numSpheres; id++ {
...
func TopologicalAnalysisModule(data []Particle, ids []int, coms []mgl32.Vec3, connected []bool, beta0 []int) {
	...
	radiusSq := tdaClusterRadius * tdaClusterRadius
	...
	for sphereID := 0; sphereID < numSpheres; sphereID++ {
		...
		// (A full TDA pipeline would use an external graph for this)
		for i := 0; i < len(group); i++ {
			...
			for j := i + 1; j < len(group); j++ {
				...
				if p1Pos.Sub(p2Pos).LenSqr() < radiusSq {
					uf.Union(indexMap[p1GlobalIdx], indexMap[p1GlobalIdx])
				}
			}
		}
