...
const (
	...
	BOUNDARY = 1.8 // Increased boundary slightly to fit the new geometry
	...
	PARTICLE_COUNT           = 80 // Must be a multiple of numRings
)
...
func updateSimulation(simData *SimulationData, deltaTime float64, projection, view mgl32.Mat4, cameraPos mgl32.Vec3) {
	...
	// We use the origin (0,0,0) as the reference point for mouse interaction,
	// so unproject to the z=0 plane passing through the origin.
	mouseWorld := unprojectMouseToWorldPlane(mouseNDC, projection, view, cameraPos, 0.0)

	// Spring settings
	const springK = 8.8  // stiffness
	const restLen = 0.35 // desired rest length for springs (matches original structure)
	...
	for i := range simData.Particles {
		...
		// The boundary is increased from 1.6 to 1.8 to fit the new shape
		if p.Pos.X() > BOUNDARY {
			p.Pos[0] = BOUNDARY
			p.Vel[0] = -p.Vel[0] * 0.6
		}
...
// --- Mock data creation (Modified for Geometric Shape) ---
func createMockData() *SimulationData {
	particles := make([]Particle, PARTICLE_COUNT)
	nextIdx := make([]int, PARTICLE_COUNT)

	const numRings = 4
	const particlesPerRing = PARTICLE_COUNT / numRings // 80 / 4 = 20

	// Define radii and Z-positions to form a barrel/cylinder-like shape
	const zStart = -1.0
	const zSpacing = 2.0 / (numRings - 1)
	const ringRadiusOuter = 1.2
	const ringRadiusInner = 0.8 // Smaller radius at the top/bottom for a tapered look

	for i := 0; i < PARTICLE_COUNT; i++ {
		ringIndex := i / particlesPerRing // 0, 1, 2, 3
		ringPos := i % particlesPerRing   // 0 to 19 within each ring

		// 1. Position particles
		z := zStart + float32(ringIndex)*zSpacing

		// Calculate radius for tapered shape (inner at ends, outer in middle)
		radius := ringRadiusInner
		if ringIndex == 1 || ringIndex == 2 {
			radius = ringRadiusOuter
		}

		angle := float32(ringPos) * (2 * math.Pi / float32(particlesPerRing))

		x := float32(radius) * float32(math.Cos(float64(angle)))
		y := float32(radius) * float32(math.Sin(float64(angle)))

		pos := mgl32.Vec3{x, y, z}

		// 2. Initial velocity (gentle rotation)
		vel := mgl32.Vec3{
			-y * 0.05,                    // Tangential velocity (X)
			x * 0.05,                     // Tangential velocity (Y)
			(rand.Float32()*0.02 - 0.01), // Small Z jitter
		}.Mul(0.1)

		// 3. Color based on ring
		var col mgl32.Vec3
		switch ringIndex {
		case 0:
			col = mgl32.Vec3{0.3, 0.3, 0.9} // Blue
		case 1:
			col = mgl32.Vec3{0.3, 0.9, 0.5} // Green
		case 2:
			col = mgl32.Vec3{0.9, 0.7, 0.3} // Yellow/Orange
		case 3:
			col = mgl32.Vec3{0.9, 0.3, 0.6} // Pink
		}
		...
		// 4. Connection Logic (NextIdx)
		// We use the single 'NextIdx' slot to alternate between connecting *within* the ring (circular)
		// and connecting *vertically* (cross-ring) to form a complete geometric mesh.

		if ringPos < particlesPerRing/2 {
			// First half of the ring links *circularly* to the next particle in the ring
			nextIdx[i] = ringIndex*particlesPerRing + (ringPos+1)%particlesPerRing
		} else {
			// Second half links *vertically* to the corresponding particle in the next ring
			if ringIndex < numRings-1 {
				// Link to the same position in the next ring
				nextIdx[i] = (ringIndex+1)*particlesPerRing + ringPos
			} else {
				// Last ring: link vertically back to the first ring (closing the cylinder vertically)
				nextIdx[i] = ringPos
			}
		}
	}
